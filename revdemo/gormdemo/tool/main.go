...
const MySQLDSN = "root:password@tcp(localhost:3306)/exert?charset=utf8mb4&parseTime=True&loc=Local"
...
func main() {
	...
	// 配置
	conf := gen.Config{
		// 查询输出路径，相对执行`go run`时的路径
		OutPath: "entities",

		// 模型输出路径，相对执行`go run`时的路径, 默认是 model
		ModelPkgPath: "models",
	...
	*/
	...
	// 指定表
	tables := []string{ "e_employee", "e_employee_mobilephone"}
	allModel, err := genModels(g, db, tables)
	if err != nil {
		panic(fmt.Errorf("get tables info fail: %v", err))
	}
